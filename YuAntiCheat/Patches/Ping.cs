...
//使用Debug构建
#if DEBUG
        __instance.text.text += $"<color={Main.ModColor}>{Main.ModName}</color><color=#00FFFF> v{Main.PluginVersion}</color>";
        __instance.text.text += $"\n<color=#FFC0CB>DEBUG:{ThisAssembly.Git.Branch}({ThisAssembly.Git.Commit})</color>";
...
#if CANARY
        __instance.text.text += $"<color={Main.ModColor}>{Main.ModName}</color><color=#00FFFF> v{Main.PluginVersion}</color>";
        __instance.text.text += $"\n<color=#FFC0CB>CANARY:{ThisAssembly.Git.Branch}({ThisAssembly.Git.Commit})</color>";
        __instance.text.text += "\n<color=#FFFF00>By</color> <color=#FF0000>Yu</color>";
#endif
...
//使用Release构建
        ...
        if (!Toggles.ShowIsSafe && Toggles.ShowIsDark)
        {
            __instance.text.text += "\n";
        }
        ...
        {
            if (Toggles.DarkMode)
                 __instance.text.text += "<color=#00BFFF>[Dark]</color>";
            else
                __instance.text.text += "<color=#00FA9A>[Light]</color>";
        }
        if (!Toggles.ShowIsDark && Toggles.ShowIsAutoExit)
        {
            __instance.text.text += "\n";
        }
        ...
        __instance.text.text += Toggles.AutoExit ? "<color=#DC143C>[AutoExit]</color>" : "<color=#1E90FF>[UnAutoExit]</color>";
